...
class SGNS(nn.Module):
    ...
    def forward(self, iword, owords, rwords_dict):
        ...
        if rwords_dict is not None:
            ...
            total_r_loss = sum([MSE_loss_fun(rvectors[i], self.previous_model[rwords_dict[rwords[i]]]) for i in range(len(rwords))])
            #print(-(oloss + nloss).mean(), 0.0001*total_r_loss)
            return(-(oloss + nloss).mean() + 0.0001*total_r_loss)
        else:
